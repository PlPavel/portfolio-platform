...
export default function CaseView({ caseData, blocks, designer }: Props) {
  const sorted = [...blocks].sort((a, b) => a.order_index - b.order_index)

  function getBlock(type: BlockType) {
    return sorted.find((b) => b.block_type === type)?.content ?? {}
  }
  ...
  return (
    <article className="max-w-2xl mx-auto px-4 py-12 space-y-10">
      ...
      <nav className="text-sm text-muted-foreground flex items-center gap-2">
        <Link href={`/portfolio/${designer.username}`} className="hover:text-foreground">
          {designer.name}
        </Link>
        <span>›</span>
      ...
      {(design.processDescription || design.imageUrls?.length) && (
        <Section title="Дизайн">
          {design.processDescription   && <Prose label="Процесс"              text={design.processDescription} />}
          {design.decisionJustification && <Prose label="Обоснование решений" text={design.decisionJustification} />}
          {design.iterations            && <Prose label="Итерации"            text={design.iterations} />}
          {design.imageUrls && design.imageUrls.length > 0 && (
            <ImageLightbox images={design.imageUrls} />
          )}
        </Section>
      )}
      ...
      {(results.metrics?.length || results.qualitativeResults) && (
        <Section title="Результаты">
          ...
          {results.qualitativeResults && <Prose label="Итоги"      text={results.qualitativeResults} />}
